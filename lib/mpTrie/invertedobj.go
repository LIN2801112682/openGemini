...
type InvertedListBlock struct {
	blk     []*InvertedItem //unused
	mpblk   map[uint64][]utils.TimePoint
	blksize uint64
}
...
func InitInvertedListBlock(mpblk map[uint64][]utils.TimePoint, blksize uint64) *InvertedListBlock {
	return &InvertedListBlock{mpblk: mpblk, blksize: blksize}
}

func (i *InvertedListBlock) Mpblk() map[uint64][]utils.TimePoint {
	return i.mpblk
}

func (i *InvertedListBlock) SetMpblk(mpblk map[uint64][]utils.TimePoint) {
	i.mpblk = mpblk
}
