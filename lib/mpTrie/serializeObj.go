...
func NewSerializeObj(data string,freq uint32, addrListlen uint64, addrListEntry *AddrListEntry, invertedListlen uint64, invertedListEntry *InvertedListEntry) *SerializeObj {
	tmp := uint64(len(data)) +FREQ_SIZE+ DEFAULT_SIZE*2 //no affection
	return &SerializeObj{
		data:        data,
		freq: 		 freq,
...
func (s *SerializeObj) UpdateSeializeObjSize() {
	res := uint64(len([]byte(s.data))) +FREQ_SIZE+ DEFAULT_SIZE*2
	if s.addrListlen != 0 {
		res += DEFAULT_SIZE * 2
	}
