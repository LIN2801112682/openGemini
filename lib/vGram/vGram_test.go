...
//	CreateIndex(*clvcDicTrie, clvcDicTrie.Qmin(), clvcDicTrie.Qmax())
//	var tests = []struct {
//		input string
//		want  int
//	}{
...
//		resLen := len(gramMatchQuery.MatchSearch(test.input, clvcDicTrie.Root(), indexRoots, clvcDicTrie.Qmin(), buffer, addrCache, invtdCache))
//		if resLen != test.want {
...
//	CreateIndex(*clvlDicTrie, clvlDicTrie.Qmin(), clvlDicTrie.Qmax())
//	var tests = []struct {
//		input string
//		want  int
//	}{
...
//	flagPass := true
//	for i, test := range tests {
//		resLen := len(gramMatchQuery.MatchSearch(test.input, clvlDicTrie.Root(), indexRoots, clvlDicTrie.Qmin(), buffer, addrCache, invtdCache))
//		if resLen != test.want {
//			t.Errorf("第%d个测试用例未通过", i)
//			flagPass = false
//		}
...
//	encode.SerializeToFile(indexTrie, indexout)
